...
function updateScrollSpy(activeElem) {
    ...
    //$('#subTabs').append('<li class="divider"></li>');
    //$('#subTabs').append('<li><a target="blank" href="https://wwwin-gitlab-sjc.cisco.com/cisco-ui/pattern-library/issues" tabindex="0"><span class="icon-bug"></span><span class="half-margin-left">Report Issue</span></a>');
    //$('#subTabs').append('<li><a target="blank" href="https://wwwin-gitlab-sjc.cisco.com/cisco-ui/pattern-library/wikis/2.0.0-Feedback" tabindex="0"><span class="icon-feedback"></span><span class="half-margin-left">Feedback</span></a>');
...
function doGlobalSearch(searchStr, forceFlag) {
    ...
    for (var ii=0;ii<allEntries.length;ii++) {
        var entry = allEntries[ii];
        if (_.startsWith(entry.name.toLowerCase(), searchStr) || forceFlag) {
            results.push(entry);
        }
    }
        ...
        _.each(value, function(result) {
            if (result.group !== '') {
                if (result.depth === "2") {
                    ...
                    str += '<a href="' + result.ref + '"><span class="text-capitalize">' + result.group + '</span>&nbsp;&#8594;&nbsp;<span class="text-capitalize text-italic">' + highlightString(result.name, searchStr) + '</span></a>';
                }
            }
        });
        str += '</div>';
